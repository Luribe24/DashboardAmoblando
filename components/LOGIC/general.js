...
async function buildTable(test) {
    ...
    test == 'A' &&  (document.body.querySelector(`.engagementWith${test}`).innerHTML = `Con interacción: <span class="valorKPI">${Math.floor(totalTime.totalWith/60)}: ${totalTime.totalWith % 60}</span>`);
    document.body.querySelector(`.engagementWithOut${test}`).innerHTML = `Sin interacción: <span class="valorKPI">${Math.floor(totalTime.totalWithout/60)}: ${totalTime.totalWithout % 60}</span>`;
